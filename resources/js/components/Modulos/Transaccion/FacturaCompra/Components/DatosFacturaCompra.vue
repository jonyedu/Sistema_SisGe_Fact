        ...
        <div class="row">
          ...
          <div class="col-lg-6 col-md-6 col-sm-12">
            <bs-text-field
              ...
              required
              :disabled="band"
              help-text="Puede consultar por cédula o apellido o razón social"
              v-model="dataFacturaCompra.cedula"
              @input="consultarcedula(this)"
              clear-button
              :external-validator="cedulaValidator"
            >
              <label>Bucar Proveedor</label>
            </bs-text-field>
          </div>
          <div class="col-lg-6 col-md-6 col-sm-12">
            <bs-text-field
...
export default {
      ...
      return {
        ...
        messages: {
          ...
          minLength: this.getMensajeValidator(
            1,
            this.$v.dataFacturaCompra.no_documento.$params.minLength.min
          ),
          maxLength: this.getMensajeValidator(
            2,
            this.$v.dataFacturaCompra.no_documento.$params.maxLength.max
          ),
        },
    ...
    noAutorizacionValidator() {
      return {
        ...
        messages: {
          ...
          minLength: this.getMensajeValidator(
            1,
            this.$v.dataFacturaCompra.no_autorizacion.$params.minLength.min
          ),
          maxLength: this.getMensajeValidator(
            2,
            this.$v.dataFacturaCompra.no_autorizacion.$params.maxLength.max
          ),
        },
  ...
  methods: {
        ...
        axios
          ...
          .then(function (response) {
            if (response.data.existe != null) {
              ...
              that.dataFacturaCompra.cedula = response.data.proveedor.cedula;
              that.dataFacturaCompra.nombre = response.data.proveedor.nombre;
    ...
    consultarcedula() {
      if (this.dataFacturaCompra.cedula.length >= 3) {
        this.searchProveedor(this.dataFacturaCompra.cedula, this);
      }
    },
    searchProveedor: _.debounce((cedula, vm) => {
      axios
        .get(
          vm.$store.state.url_prefix +
        ...
        .then((response) => {
          ...
          if (response.data.proveedor == null) {
            ...
            vm.dataFacturaCompra.proveedor_id = response.data.proveedor.id;
            vm.dataFacturaCompra.cedula = response.data.proveedor.cedula;
            vm.dataFacturaCompra.nombre = response.data.proveedor.nombre;
            vm.dataFacturaCompra.apellido = response.data.proveedor.apellido;
            vm.dataFacturaCompra.direccion = response.data.proveedor.direccion;
            vm.dataFacturaCompra.telefono = response.data.proveedor.telefono;
            vm.dataFacturaCompra.razon_social =
              response.data.proveedor.razon_social;
          }
        });
    }, 350),
      ...
      axios
        ...
        .then(function (response) {
          if (response.data.proveedor == null) {
            ...
            that.dataFacturaCompra.cedula = response.data.proveedor.cedula;
            that.dataFacturaCompra.nombre = response.data.proveedor.nombre;
            that.dataFacturaCompra.apellido = response.data.proveedor.apellido;
            that.dataFacturaCompra.direccion =
              response.data.proveedor.direccion;
            that.dataFacturaCompra.telefono = response.data.proveedor.telefono;
            that.dataFacturaCompra.razon_social =
              response.data.proveedor.razon_social;
          }
        })
      ...
      switch (opc) {
        case 1:
          return (this.minLengthErrorMsg =
            "Este campo debe tener al menos" + value + " caracteres.");
        case 2:
